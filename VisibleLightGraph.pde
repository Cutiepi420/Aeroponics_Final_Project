...
void setup()
{
  ...
  lineChart.setData(time, Visible);
   
  // Axis formatting and labels.
  lineChart.showXAxis(true); 
  lineChart.showYAxis(true); 
  float miny = Visible[0];
  ...
  lineChart.setXAxisLabel("\n Time "+  
                            "(Minutes)");
  lineChart.setYAxisLabel("Visible Light Intensity (Relative Units) \n");
...
void draw()
{
  ...
  fill(120);
  textSize(36);
  text("Visible Light Fluctuation for "+titletime, 150,30);
  textSize(22);
